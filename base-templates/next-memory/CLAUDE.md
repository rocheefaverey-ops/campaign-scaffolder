# Next.js + Card Memory Game Campaign Template

Core Next.js 16 project with a built-in React card memory mini-game (no external engine). **Contains only mandatory code.** Optional module code lives in `../modules/`.
...
- **Game:** Built-in card memory mini-game (pure React, no game engine)
...
4. **Memory game** is a built-in React component (no external engine). Game state managed via React context + localStorage.
