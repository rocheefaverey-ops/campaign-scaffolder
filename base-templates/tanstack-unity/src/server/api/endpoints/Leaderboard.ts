...
function buildMockLeaderboard(type: LeaderboardInput['type'], offset: number, limit: number) {
  const fixture = leaderboardFixture as LeaderboardData;
  if (fixture.entries?.length) {
    const entries = fixture.entries.slice(offset, offset + limit);
    return {
      entries,
      personalBest: fixture.personalBest ?? null,
      total: fixture.total ?? fixture.entries.length,
    };
  }
