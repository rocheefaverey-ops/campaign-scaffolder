...
export function UnityGame({ children, buildBaseUrl, isLocal = false }: UnityGameProps) {
  ...
  const addEventListener = useCallback((eventName: string, callback: UnityEventCallback) => {
    ...
    // Store the array directly so Unity's _WebCallback can iterate it.
    // The array reference is stable — splice/push mutations are reflected automatically.
    window.unityEventMap.set(eventName, cbs);
  }, []);
