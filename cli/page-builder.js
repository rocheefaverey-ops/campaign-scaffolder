 ...
 */
export function buildBlockDrivenLanding(blocks) {
  ...
  return [
    ...
    headerBlock ? `      <HeaderChrome leftSlot="${headerBlock.settings.leftSlot}" rightSlot="${headerBlock.settings.rightSlot}" />` : '',
    brandBlock ? `      <BrandChip image={cape.brandChip?.image} size="${brandBlock.settings.size}" />` : '',
    titleBlock
      ...
      : '',
    ctaButtonsJsx ? `      <CtaGroup buttons={${ctaButtonsJsx}} />` : '',
    footerBlock ? '      <FooterLinkList links={cape.footerLinks ?? []} />' : '',
    '    </Background>',
    '  );',
    '}',
  ].filter((l) => l !== '').join('\n');
...
function renderCtaButtons(settings) {
