...
export function buildBlockDrivenPage(pageId, pageType, blocks, options = {}) {
  ...
  return [
    ...
    isWaitForEngineVideoPage ? [
      ...
      '    // preloads (fullBoot resolves). On a hard boot error OR a stalled boot,',
      '    // surface a manual Continue button so the flow never wedges.',
      '    if (booted.current) return;',
      '    // Mock mode: no real engine to wait for — show the loading video briefly,',
      '    // then advance. Prevents an infinite loading screen on local/dev runs.',
      "    if (process.env.NEXT_PUBLIC_GAME_MOCK === 'true') {",
      '      booted.current = true;',
      '      const t = window.setTimeout(() => goToGame(), 2500);',
      ...
      '    // Safety net: if fullBoot never resolves or rejects, reveal Continue.',
      '    const stallTimer = window.setTimeout(() => setCanContinue(true), 20000);',
      '    void unity.fullBoot()',
      '      .then(() => goToGame())',
      '      .catch((error) => {',
