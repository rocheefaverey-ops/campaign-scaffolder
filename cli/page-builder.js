...
function routeForExit(exit, routeMap = {}) {
  if (!exit) return '/';
  if (String(exit).startsWith('/')) return exit;
  return routeMap[exit] ?? DEFAULT_BLOCK_ROUTES[exit] ?? `/${exit}`;
}
...
function renderBlock(block, ctx) {
  ...
  switch (block.name) {
    ...
    case 'video-player':
      return `      <VideoPlayer src={cape.video?.url ?? cape.video} muted={${s.muted !== false}} loop={${Boolean(s.loop)}} onEnded={() => router.push(${jsString(routeForExit(s.exit ?? 'game', ctx.routeMap))})} />`;
    case 'skip-control':
    ...
    case 'reveal-cta':
      return `      <RevealCta label={cape.ctaLabel ?? 'Continue'} onClick={() => router.push(${jsString(routeForExit(s.exit ?? 'game', ctx.routeMap))})} />`;
