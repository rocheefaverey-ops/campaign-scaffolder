  ...
  switch (block.name) {
    ...
    case 'title-block': {
      ...
      const subtitleExpr = ctx.stepFlow
        ...
        : (s.showSubtitle ? ' subtitle={cape.subtitle}' : '');
      return `      <TitleBlock${kickerExpr} title={${titleExpr}}${subtitleExpr} />`;
    }
    case 'body-copy':
      return ctx.stepFlow
        ? '      <BodyCopy text={currentStep.description ?? cape.body ?? cape.subline ?? ""} />'
        : '      <BodyCopy text={cape.body ?? cape.subline ?? ""} />';
    case 'cta-group':
    ...
    case 'tagline':
      return "      <Tagline text={cape.tagline || 'Loading game...'} />";
...
function renderRegisteredResultButtons(buttons, ctx = {}) {
  ...
  const add = (exit, label, variant = 'secondary') => {
    ...
    out.push(`{ label: ${jsString(label)}, variant: '${variant}', onClick: () => router.navigate({ to: '${route}' as never }) }`);
  };
