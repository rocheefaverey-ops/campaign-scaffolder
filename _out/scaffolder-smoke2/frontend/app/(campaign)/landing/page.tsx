...
export default function LandingPage() {
  ...
  const nextAfterLanding = '/intro-video';
  const nextAfterOnboardingRaw = '/loading-video';
  const nextAfterOnboarding = nextAfterOnboardingRaw.startsWith('{{')
    ? nextAfterLanding
    : nextAfterOnboardingRaw;

  // Show onboarding only once. Returning players skip it.
  const handlePlay = () =>
    navigate(onboardingCompleted ? nextAfterOnboarding : nextAfterLanding);
      ...
      <div className="campaign-shell">
        ...
        <div className="campaign-actions" style={{ animation: 'fadeIn 0.5s 0.28s ease both' }}>
          ...
          {(showLeaderboard || hasPlayed) && (
            <Button variant="secondary" className="w-full" size="lg" onClick={() => navigate('/landing')}>
