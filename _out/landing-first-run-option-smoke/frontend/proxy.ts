...
export default function proxy(request: NextRequest) {
  ...
  const csp = `
    default-src 'self';
    script-src 'self' 'nonce-${nonce}' 'strict-dynamic' 'unsafe-inline'${isDev ? " 'unsafe-eval'" : ''} 'wasm-unsafe-eval'${unityOrigin ? ` ${unityOrigin}` : ''} https://www.googletagmanager.com https://storage.googleapis.com https://*.lwcf.nl https://*.lwcf5.nl https://lw-*.lwdev.nl;
